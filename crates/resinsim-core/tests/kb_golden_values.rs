...
fn kb100_premium_black_cure_depth() {
    ...
    let cd = CureCalculator::cure_depth(PenetrationDepth::new(170.0).unwrap(), Energy::new(10.0).unwrap(), Energy::new(5.0).unwrap());
    assert!((cd.value() - 117.83).abs() < 0.5, "KB-100 Premium Black: got {:.2}", cd.value());
}
...
fn kb100_premium_white_cure_depth() {
    ...
    let cd = CureCalculator::cure_depth(PenetrationDepth::new(350.0).unwrap(), Energy::new(20.0).unwrap(), Energy::new(6.87).unwrap());
    assert!((cd.value() - 374.0).abs() < 1.0, "KB-100 Premium White: got {:.2}", cd.value());
}
...
fn kb100_strong_x_cure_depth() {
    ...
    let cd = CureCalculator::cure_depth(PenetrationDepth::new(200.0).unwrap(), Energy::new(20.0).unwrap(), Energy::new(9.88).unwrap());
    assert!((cd.value() - 141.0).abs() < 1.0, "KB-100 Strong-X: got {:.2}", cd.value());
}
...
fn kb100_tough_x_cure_depth() {
    ...
    let cd = CureCalculator::cure_depth(PenetrationDepth::new(270.0).unwrap(), Energy::new(50.0).unwrap(), Energy::new(24.20).unwrap());
    assert!((cd.value() - 196.0).abs() < 1.0, "KB-100 Tough-X: got {:.2}", cd.value());
}
...
fn kb101_pr48_365nm() {
    ...
    let cd = CureCalculator::cure_depth(PenetrationDepth::new(42.0).unwrap(), Energy::new(50.0).unwrap(), Energy::new(18.3).unwrap());
    assert!((cd.value() - 42.2).abs() < 0.5, "KB-101 PR48@365nm: got {:.2}", cd.value());
}

#[test]
fn kb101_veroclear_405nm() {
    // KB-101: VeroClear at 405nm, Dp=568µm, Ec=6.9 mJ/cm²
    let cd = CureCalculator::cure_depth(PenetrationDepth::new(568.0).unwrap(), Energy::new(50.0).unwrap(), Energy::new(6.9).unwrap());
    assert!((cd.value() - 1125.0).abs() < 5.0, "KB-101 VeroClear@405nm: got {:.2}", cd.value());
}

#[test]
fn kb101_formlabs_clear_405nm() {
    // KB-101: Formlabs Clear at 405nm, Dp=192µm, Ec=12.6 mJ/cm²
    let cd = CureCalculator::cure_depth(PenetrationDepth::new(192.0).unwrap(), Energy::new(30.0).unwrap(), Energy::new(12.6).unwrap());
    // Cd = 192 × ln(30/12.6) = 192 × 0.868 = 166.6 µm
    assert!((cd.value() - 166.6).abs() < 1.0, "KB-101 Formlabs Clear: got {:.2}", cd.value());
}
...
fn kb102_nist_pr48_405nm() {
    ...
    let cd = CureCalculator::cure_depth(PenetrationDepth::new(69.3).unwrap(), Energy::new(50.0).unwrap(), Energy::new(17.9).unwrap());
    assert!((cd.value() - 71.2).abs() < 1.0, "KB-102 NIST PR48: got {:.2}", cd.value());
}
...
fn kb110_standard_fep_50mm_square() {
    ...
    assert!((f.value() - 32.5).abs() < 0.01, "KB-110 FEP 50mm²: got {:.2}", f.value());
}
...
fn kb110_acf_50mm_square() {
    ...
    assert!((f.value() - 30.0).abs() < 0.01, "KB-110 ACF: got {:.2}", f.value());
}
...
fn kb110_thick_fep_50mm_square() {
    ...
    assert!((f.value() - 45.0).abs() < 0.01, "KB-110 thick FEP: got {:.2}", f.value());
}
...
fn kb110_full_saturn_plate() {
    ...
    assert!((f.value() - 106.08).abs() < 0.1, "KB-110 full Saturn: got {:.2}", f.value());
}
...
fn kb114_single_04mm_tip() {
    ...
    assert!((cap.value() - 3.77).abs() < 0.01, "KB-114 single tip: got {:.2}", cap.value());
}
...
fn kb114_ten_supports() {
    ...
    assert!((cap.value() - 37.7).abs() < 0.1, "KB-114 ten supports: got {:.2}", cap.value());
}
...
fn kb114_suction_sealed_30mm_cup() {
    ...
    assert!((f.value() - 71.3).abs() < 0.1, "KB-114 sealed 30mm: got {:.2}", f.value());
}
...
fn kb114_suction_drained_is_zero() {
    ...
    assert!(f.value().abs() < 1e-6, "KB-114 drained: got {:.6}", f.value());
}
...
fn kb120_saturn1_center_brightest() {
    ...
    assert!((center - 1.17).abs() < 0.01, "KB-120 center: got {:.3}", center);
    assert!((corner - 0.83).abs() < 0.01, "KB-120 corner: got {:.3}", corner);
}
...
fn kb120_saturn2_less_variation() {
    ...
    let corner = UniformityCalculator::intensity_factor(0.0, 0.0, &p);
    assert!((center - 1.11).abs() < 0.01, "KB-120 S2 center: got {:.3}", center);
    assert!((corner - 0.89).abs() < 0.01, "KB-120 S2 corner: got {:.3}", corner);
}
...
fn kb131_mrazek_sculpt_resin_deflection() {
    ...
    assert!((dz - 434.8).abs() < 0.1, "KB-131 Sculpt resin: got {:.1}", dz);
}
...
fn kb141_82pct_drop_25_to_50() {
    ...
    assert!((ratio - 0.18).abs() < 0.03, "KB-141 82% drop: ratio={:.3}", ratio);
}
...
fn kb141_viscosity_at_30c() {
    ...
    assert!((ratio - 0.775).abs() < 0.02, "KB-141 30°C ratio: got {:.3}", ratio);
}
...
fn kb141_elegoo_abs_like_viscosity() {
    ...
    assert!(mu > 20.0 && mu < 50.0, "KB-141 ABS-Like at 50°C: got {:.1}", mu);
}
...
fn kb150_temperature_at_10min() {
    ...
    assert!((t.value() - 25.9).abs() < 0.1, "KB-150 10min: got {:.1}", t.value());
}
...
fn kb150_temperature_at_20min() {
    ...
    assert!((t.value() - 28.3).abs() < 0.1, "KB-150 20min: got {:.1}", t.value());
}
...
fn kb150_temperature_at_40min() {
    ...
    assert!((t.value() - 30.6).abs() < 0.1, "KB-150 40min: got {:.1}", t.value());
}

#[test]
fn kb150_steady_state() {
    // KB-150: t→∞ → T=32.0°C
    let t = ThermalCalculator::vat_temperature(22.0, 10.0, ThermalTimeConstant::new(1200.0).unwrap(), 100000.0);
    assert!((t.value() - 32.0).abs() < 0.1, "KB-150 steady state: got {:.1}", t.value());
}
...
fn kb172_force_linearity() {
    ...
    for i in 1..6 {
        ...
        assert!((ratio - ratio_first).abs() < 1e-4,
...
fn kb173_sealed_20mm_cup_force() {
    ...
    let f = PeelForceCalculator::suction_force(50.0, CrossSectionArea::new(254.5).unwrap());
    assert!((f.value() - 12.73).abs() < 0.1, "KB-173 sealed 20mm: got {:.2}", f.value());
}
...
fn kb173_sealed_dominates_peel() {
    ...
    let wall_peel = PeelForceCalculator::peel_force(13.0, CrossSectionArea::new(59.7).unwrap(), 1.0);
    let suction = PeelForceCalculator::suction_force(50.0, CrossSectionArea::new(254.5).unwrap());
    let ratio = suction.value() / wall_peel.value();
    assert!(ratio > 5.0, "KB-173: suction should dominate peel by >5×, got {ratio:.1}×");
}
