...
```gherkin
Scenario: UAT-1 happy-path pairing — in-range recipe runs through full simulation
  Given a printer profile "P" with:
    | field                   | min   | max   |
    | layer_height_range_um   | 20.0  | 100.0 |
    | exposure_range_sec      | 1.0   | 60.0  |
    | lift_speed_range_mm_min | 10.0  | 200.0 |
  And printer "P" has bottom_layer_count_max = 15
  ...
  When SimulationRunner.run_from_areas is invoked on a non-trivial area vector
  Then validate_pairing(P, R.recipe()) returns Ok(())
  And SimulationRunner proceeds through slice_areas → predict_layer for every layer
...
```gherkin
Scenario: UAT-2 boundary values accepted — inclusive comparison at range edges
  ...
  Then it returns Ok(())
  And the same is true when recipe.lift_speed_mm_min equals the range max of lift_speed_range_mm_min

  Given a printer "P2" pinned to a single layer height (layer_height_range_um min 50.0 max 50.0)
