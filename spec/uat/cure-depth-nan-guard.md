...
Given a resin profile with a critical energy Ec that is zero or non-finite
  (e.g. injected via a bypass path or future serialisation gap)
When the Beer-Lambert cure depth calculator runs for a layer
Then the system fails loudly with a clear diagnostic message referencing
  critical_energy
...
Given a print with a uniformity profile where the intensity factor computation
  produces a non-finite value (e.g. via NaN plate dimensions)
When the uniformity-corrected cure depth is computed
Then the system panics with a clear "scale factor must be positive and finite"
  message from Energy::scale
